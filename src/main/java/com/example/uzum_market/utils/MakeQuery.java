...
import com.example.uzum_market.dto.FilterDTO;
import com.example.uzum_market.dto.FilterField;
import com.example.uzum_market.dto.FilterFieldValue;
    ...
    private static void addFilterQuery(StringBuilder sb, FilterDTO filter, boolean isFiltering) {
        if (isFiltering) {
            ...
            for (int i = 0; i < filterFields.size(); i++) {
                if (i == 0) sb.append(filter.getFilterOperator());
                sb.append("( ");
                FilterField field = filterFields.get(i);
                ComparatorTypeEnum comparatorType = field.getComparatorType();
                FilterFieldValue fieldValue = field.getValue();
                sb.append(filterFields);
                openedByColumnType(sb, comparatorType, fieldValue, field);
                sb.append(" )");
            }
            sb.append(") ");
        }
